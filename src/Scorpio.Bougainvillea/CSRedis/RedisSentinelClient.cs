...
namespace CSRedis
{
    ...
    /// </summary>
    internal partial class RedisSentinelClient : IDisposable
    {
        private const int _defaultPort = 26379;
        private const bool _defaultSSL = false;
        private const int _defaultConcurrency = 1000;
        private const int _defaultBufferSize = 1024;
        private readonly RedisConnector _connector;
        private readonly SubscriptionListener _subscription;
        ...
        public RedisSentinelClient(string host, int port, bool ssl)
            : this(new RedisSocket(ssl), new DnsEndPoint(host, port), _defaultConcurrency, _defaultBufferSize)
        { }
