...
namespace CSRedis
{
    ...
    /// </summary>
    internal partial class RedisClient : IRedisClientSync, IRedisClientAsync
    {
        const int _defaultPort = 6379;
        const bool _defaultSSL = false;
        const int _defaultConcurrency = 1000;
        const int _defaultBufferSize = 10240;
        ...
        bool _streaming;
        internal RedisReader Reader => _connector?._io?.Reader;
        ...
        public RedisClient(string host, int port, bool ssl)
            : this(host, port, ssl, _defaultConcurrency, _defaultBufferSize)
        { }
        ...
        public RedisClient(EndPoint endpoint, bool ssl)
            : this(endpoint, ssl, _defaultConcurrency, _defaultBufferSize)
        { }
        ...
        public RedisClient(string host, int port, int asyncConcurrency, int asyncBufferSize)
            : this(host, port, _defaultSSL, asyncConcurrency, asyncBufferSize)
        { }
        ...
        public RedisClient(EndPoint endpoint, int asyncConcurrency, int asyncBufferSize)
            : this(endpoint, _defaultSSL, asyncConcurrency, asyncBufferSize)
        { }
        ...
        internal RedisClient(IRedisSocket socket, EndPoint endpoint)
            : this(socket, endpoint, _defaultConcurrency, _defaultBufferSize)
        { }
        ...
        {
            StreamTo(destination, _defaultBufferSize, func);
        }
