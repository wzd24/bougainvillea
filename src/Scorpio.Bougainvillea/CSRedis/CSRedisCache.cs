...
namespace CSRedis
{
    internal class CSRedisCache : IDistributedCache
    {
        private readonly ICSRedisClient _redisClient;
        public CSRedisCache(ICSRedisClient redisClient)
        {
        ...
        private const string _setScript = (@"
        ...
        public void Set(string key, byte[] value, DistributedCacheEntryOptions options)
        {
            ...
            var result = _redisClient.Eval(_setScript, key,
                new object[]
                {
                        absoluteExpiration?.Ticks ?? _notPresent,
        ...
        public async Task SetAsync(string key, byte[] value, DistributedCacheEntryOptions options, CancellationToken token = default(CancellationToken))
        {
            ...
            var absoluteExpiration = GetAbsoluteExpiration(creationTime, options);
            await _redisClient.EvalAsync(_setScript, key,
                new object[]
                {
                        absoluteExpiration?.Ticks ?? _notPresent,
                        options.SlidingExpiration?.Ticks ?? _notPresent,
        ...
        private byte[] GetAndRefresh(string key, bool getData)
        {
            ...
            if (getData)
            {
                var ret = _redisClient.HMGet<byte[]>(key, _absoluteExpirationKey, _slidingExpirationKey, _dataKey);
            ...
            {
                results = _redisClient.HMGet(key, _absoluteExpirationKey, _slidingExpirationKey);
            }
        ...
        private async Task<byte[]> GetAndRefreshAsync(string key, bool getData, CancellationToken token = default(CancellationToken))
        {
            ...
            if (getData)
            {
                var ret = await _redisClient.HMGetAsync<byte[]>(key, _absoluteExpirationKey, _slidingExpirationKey, _dataKey);
                results = new object[] { ret[0] == null ? null : Encoding.UTF8.GetString(ret[0]), ret[1] == null ? null : Encoding.UTF8.GetString(ret[1]), value = ret[2] };
            }
            else
            {
                results = await _redisClient.HMGetAsync(key, _absoluteExpirationKey, _slidingExpirationKey);
            }
        ...
        private void MapMetadata(object[] results, out DateTimeOffset? absoluteExpiration, out TimeSpan? slidingExpiration)
        {
            ...
            if (long.TryParse(results[0]?.ToString(), out var absoluteExpirationTicks) && absoluteExpirationTicks != _notPresent)
            {
                absoluteExpiration = new DateTimeOffset(absoluteExpirationTicks, TimeSpan.Zero);
            }
            if (long.TryParse(results[1]?.ToString(), out var slidingExpirationTicks) && slidingExpirationTicks != _notPresent)
            {
