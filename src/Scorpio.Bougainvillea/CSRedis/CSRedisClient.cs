...
namespace CSRedis
{
    ...
    /// 
    /// </summary>
    internal partial class CSRedisClient : IDisposable, ICSRedisClient,ISingletonDependency
    {
        /// <summary>
        /// 按 key 规则分区存储
        /// </summary>
        public ConcurrentDictionary<string, RedisClientPool> Nodes { get; } = new ConcurrentDictionary<string, RedisClientPool>();
        private int _nodesIndexIncrement = -1;
        ...
        private readonly DateTime _dt1970 = new DateTime(1970, 1, 1);
        ...
        internal string SerializeObject(object value)
        {
            ...
            return JsonConvert.SerializeObject(value, this._jsonSerializerSettings());
        }
        internal T DeserializeObject<T>(string value)
        {
            ...
            return JsonConvert.DeserializeObject<T>(value, this._jsonSerializerSettings());
        }
        ...
        private bool _backgroundGetSentinelMasterValueIng = false;
        private readonly object _backgroundGetSentinelMasterValueIngLock = new object();
        bool BackgroundGetSentinelMasterValue()
        {
            ...
            var ing = false;
            if (_backgroundGetSentinelMasterValueIng == false)
            {
                lock (_backgroundGetSentinelMasterValueIngLock)
                {
                    if (_backgroundGetSentinelMasterValueIng == false)
                    {
                        _backgroundGetSentinelMasterValueIng = ing = true;
                    }
                }
            }
                    ...
                    while (true)
                    {
                        ...
                        try
                        {
                            ...
                            if (pool.CheckAvailable())
                            {
                                ...
                                _backgroundGetSentinelMasterValueIng = false;
                                return;
                            }
                        }
        ...
        bool TryAddNode(string nodeKey, RedisClientPool pool)
        {
            ...
            {
                var nodeIndex = Interlocked.Increment(ref _nodesIndexIncrement);
                ...
                Interlocked.Decrement(ref _nodesIndexIncrement);
            }
            return false;
        }
        ...
        #region crc16
        private static readonly ushort[] _crc16tab = {
        ...
        public static ushort GetClusterSlot(string key)
        {
            ...
            for (int i = 0; i < count; i++)
                crc = ((crc << 8) ^ _crc16tab[((crc >> 8) ^ blob[offset++]) & 0x00FF]) & 0x0000FFFF;
        ...
        /// <returns></returns>
        public object[] StartPipe(Action<ICSRedisClientPipe<string>> handler)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public ICSRedisClientPipe<string> StartPipe()
        {
        ...
        internal partial class NodeServerManagerProvider
        {
            private CSRedisClient _csredis;
...
return 0", $"CSRedisPSubscribe{psubscribeKey}", "", trylong.ToString());
        ...
        public void Get(string key, Stream destination, int bufferSize = 1024)
        {
            ExecuteScalar(key, (c, k) =>
            {
                ...
                c.Value.Reader.ReadBulkBytes(destination, bufferSize, true);
                return true;
            });
        }
        /// <summary>
        ...
        public bool BfReserve(string key, double errorRate, long capacity, int expansion = 2, bool nonScaling = false) =>
        ...
        public bool CmsInitByProb(string key, double error, double probability) =>
        ...
        public bool TopkReserve(string key, long topk, long width, long depth, double decay) =>
        ...
        public (long k, long width, long depth, double decay) TopkInfo(string key) =>
    ...
    internal class CSRedisClientLock : IDisposable
    {
...
return 0", _name, _value)?.ToString() == "1";
    ...
    /// 
    /// </summary>
    internal enum KeyType { None, String, List, Set, ZSet, Hash, Stream }

    /// <summary>
    /// 
    /// </summary>
    internal enum InfoSection { Server, Clients, Memory, Persistence, Stats, Replication, CPU, CommandStats, Cluster, Keyspace }

    /// <summary>
    /// 
    /// </summary>
    internal enum ClientKillType { normal, slave, pubsub }
}
